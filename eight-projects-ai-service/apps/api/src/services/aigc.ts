...
function record(capability: string, input: unknown, output: unknown, usage: unknown, conversationId: string | null) {
  const id = uid('job-');
  openDb().run('INSERT INTO aigc_jobs VALUES (?,?,?,?,?,?,?)', id, capability, J.str(input), J.str(output), J.str(usage), nowIso(), conversationId);
  return id;
}
...
export async function summarize(conversationId: string) {
  ...
  const id = record('summary', { conversationId }, r.value, r.usage, conversationId);
...
export async function classify(conversationId: string) {
  ...
  const id = record('classify', { conversationId }, r.value, r.usage, conversationId);
  return { id, ...r.value, usage: r.usage };
}
...
export async function extractTicket(conversationId: string) {
  ...
  const id = record('ticket-extract', { conversationId }, r.value, r.usage, conversationId);
  return { id, ...r.value, usage: r.usage };
}
...
export async function rewrite(text: string, style: string, persona: string) {
  ...
  const id = record('rewrite', { text, style }, r.value, r.usage, null);
  return { id, ...r.value, usage: r.usage };
}
...
export async function extractFaq(text: string, max = 8) {
  ...
  const id = record('faq-extract', { length: text.length }, r.value, r.usage, null);
  return { id, ...r.value, usage: r.usage };
}
...
export async function similarQuestions(question: string, n = 6) {
  ...
  const id = record('similar-questions', { question }, r.value, r.usage, null);
  return { id, ...r.value, usage: r.usage };
}
...
export async function semanticQc(conversationId: string) {
  ...
  record('semantic-qc', { conversationId }, r.value, r.usage, conversationId);
...
export async function vocClassify(items: { messageId: string; text: string }[]) {
  ...
  record('voc-classify', { count: items.length }, { count: r.value.items.length }, r.usage, null);
...
export async function vocAsk(question: string, stats: unknown, samples: string[]) {
  ...
  record('voc-ask', { question }, r.value, r.usage, null);
  return { ...r.value, usage: r.usage };
}
...
export async function simulateOutbound(script: string, goal: string, contacts: { name: string; phone: string }[]) {
  ...
  record('outbound-simulate', { goal, contacts: contacts.length }, r.value, r.usage, null);
