...
建议路线：`总览` → `在线机器人` 发一句「订单 20260918000123 的快递三天没动了」看九阶段轨迹 → `在线客服` 接管待接入会话并用「AI 建议」 → `Agent Studio` 改白名单后试跑 / 跑 Benchmark → `Mind Studio` 新建资料并发布 → `智能质检` / `客户之声` 运行分析 → `数据大屏`。
...
pnpm build       # 生产构建
pnpm e2e         # Playwright（需 dev 服务已启动、系统 Chrome）；31 条用例，含 8 条真实模型业务流，断言 0 控制台错误
```
...
- [架构说明](docs/ARCHITECTURE.md) · [执行链](docs/EXECUTION-CHAIN.md) · [Loop Engineering 记录](docs/LOOP-LOG.md) · [拆仓库](docs/EXTRACT-TO-NEW-REPO.md)
