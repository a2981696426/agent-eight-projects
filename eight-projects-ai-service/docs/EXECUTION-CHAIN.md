...
| 9 | 最终回复 | 升级时生成带优先级与预计响应时窗（P0 15 分钟 / P1 2 小时 / P2 1 工作日）的转接话术；追问时用追问；否则用推理话术并附自动执行结果；同时生成给坐席看的内部备注 | `reply` 阶段 | 否 |
...
- 机器人接待（`controller = bot`）：`auto_reply` → 追加机器人消息；`human_confirm` / `escalate` → 会话转 `waiting_human`，写入转接话术给用户，并以系统内部消息保存完整内部备注供坐席接续。
