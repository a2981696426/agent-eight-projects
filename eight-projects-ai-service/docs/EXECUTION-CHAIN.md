...
| 8 | 自主处理 / 人工确认 / 升级 | 用户要求人工 → 升级；L3 → 升级（safety/legal 为 P0，否则 P1）；机器人连续轮次超阈值 → 升级；模型提案 handoff → 升级；**签发白名单**（`ChainContext.whitelist`，按渠道范围：自有渠道全时段 / 渠道平台非人工时段；无生效版本默认拒绝）允许(场景, 风险) 且 风险 ≤ min(Agent 上限, 场景包上限) 且 动作在场景包允许列表 → 自主回复（版本记入 `autonomy.whitelistVersion`）（允许时执行 `create_ticket` → 工具 `cases.create` 拆本地子案件，不写 DMS）；否则人工确认（VIP/紧急 P1，其余 P2）。非自主决策在宿主侧创建/追加 **人工接续任务**（已完成阶段、证据、缺项、候选话术、下一步） | `autonomy` 阶段 | 否 |
| 9 | 最终回复 | 产出两份文本：`reply.text` = **实际对客发送**（自主回复 = 候选话术并附自动执行结果；人工确认 = 带优先级与**预计人工响应时窗**的等待核实提示；升级 = 转接话术 + 时窗），`reply.candidate` = 推理阶段的候选话术，供坐席审核采用；时窗由 `ChainContext.responseWindow` 按工作日历与档位注入（P0 15 分钟 / P1 2 工作小时 / P2 1 工作日），只表示预计开始接续；**医疗边界**（`medical` / `medical_advice_in_draft`）→ `kind=boundary`，text 与 candidate 都替换为固定文案（紧急版含 120），不含模型内容 | `reply` 阶段 | 否 |
